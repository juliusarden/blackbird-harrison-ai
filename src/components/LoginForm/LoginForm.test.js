...
test("Password should be 8 or more characters", () => {
  expect(validatePassword("aaa")).toBe(
    "Password should be 8 or more characters"
  );
});

test("Password should contains minimum 1 character for both uppercase and lowercase letter", () => {
  expect(validatePassword("aaabbbcc")).toBe(
    "Password should contains minimum 1 character for both uppercase and lowercase letter"
  );
  expect(validatePassword("AAABBBCC")).toBe(
    "Password should contains minimum 1 character for both uppercase and lowercase letter"
  );
